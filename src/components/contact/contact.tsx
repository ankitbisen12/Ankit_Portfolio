...
import { Mail, Linkedin, Download, SendHorizonal, Loader } from 'lucide-react';
...
const Contact = () => {
    ...
    const onSubmit = async (formInput: { name: string; email: string; message: string }) => {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-black/50 rounded-2xl p-6 border border-gray-900 lg:col-span-2">
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <button className="w-full px-2 lg:px-8 py-2 bg-purple-900 text-white border border-gray-800 shake-vertical font-medium rounded-lg cursor-pointer flex items-center justify-center gap-2" type="submit" suppressHydrationWarning={true}>Send Message {loading ? <Loader size={16} /> : <SendHorizonal size={16} />}</button>
                        </form>
                    </div>
                </div>
            </div>
